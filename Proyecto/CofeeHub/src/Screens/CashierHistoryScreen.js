...
import { getHistorialPagos } from '../Services/api';
...
const CashierHistoryScreen = ({ onNavigate, onOpenMenu, token }) => {
  const [pagos, setPagos] = useState([]);
  const [cargando, setCargando] = useState(true);

  useEffect(() => {
    getHistorialPagos(token)
      .then(setPagos)
      .catch(() => {})
  ...
  return (
    <View style={styles.container}>
      ...
      {cargando ? (
        <ActivityIndicator color={colors.naranja} style={{ marginTop: 24 }} />
      ) : pagos.length === 0 ? (
