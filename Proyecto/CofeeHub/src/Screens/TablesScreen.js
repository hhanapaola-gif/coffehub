...
const TablesScreen = ({ onNavigate, token, onSelectTable }) => {
  ...
  useEffect(() => {
    getMesas(token)
      .then((mesas) => setTables(mesas.map((m) => ({ id: m.id, available: m.estatus }))))
      .catch(() => {})
  ...
  return (
    <View style={styles.container}>
      ...
      {cargando && <ActivityIndicator color="#fff" style={{ marginTop: 20 }} />}
