...
const OrderTrackingScreen = ({ onNavigate, token, pedido, onPedidoActualizado }) => {
  ...
  useEffect(() => {
    ...
    const intervalo = setInterval(async () => {
      try {
        const actualizado = await getPedido(token, pedido.id);
        onPedidoActualizado(actualizado);
        setStep(ESTATUS_A_PASO[actualizado.estatus] || 0);
      } catch (e) {
