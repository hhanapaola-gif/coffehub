...
const KitchenStatusScreen = ({ onNavigate, pedido }) => {
  if (!pedido) {
    return (
  ...
  const handleAvanzar = () => {
    ...
    Alert.alert('Estado actualizado', `El pedido #${pedido.folio} ahora está "${SIGUIENTE_LABEL[pedido.estatus]}".`, [
      { text: 'Aceptar', onPress: () => onNavigate('kitchen') },
    ]);
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.footer}>
        {siguienteEstatus ? (
          <TouchableOpacity style={styles.confirmButton} onPress={handleAvanzar}>
